...
function  ModeSwitch(){
  ...
  useEffect(() => {
    ...
    }else if (pathname.startsWith("/chat") || pathname === "/" || pathname.startsWith("/new-message")){
  ...
  return (
    <div>
      <button onClick={() => _setMode(ChatMode.CHAT)}>
        <ChatIcon color={mode === ChatMode.CHAT ? "#0E76FD" : "#CBD5E0"} />
      </button>
      &nbsp;&nbsp;
      <Switch  isChecked={mode === ChatMode.WATCH} id="mode-switch" colorScheme="blackAlpha" color="blue" onChange={() => _setMode(mode === ChatMode.CHAT ? ChatMode.WATCH : ChatMode.CHAT)} />
      &nbsp;&nbsp;
      <button onClick={() => _setMode(ChatMode.WATCH)}>
